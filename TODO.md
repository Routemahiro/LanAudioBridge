...
### Phase1: メイン画面のステータス中心化 (作業中)
目的: 「セットアップ画面」→「動作状況ダッシュボード」への転換。普段見るメイン画面に、本当に必要な情報だけを表示する。

- [ ] メイン画面レイアウトの再構成
  - [ ] ヘッダー: モード切替（受信/送信）＋ 接続状態インジケーター
  - [ ] センター: リアルタイム監視エリア（モード別に切替）
  - [ ] フッター: ステータスバー（既存を改善）

- [ ] 受信モードのメイン画面
  - [ ] 接続状態を大きく表示（「接続待ち」「受信中」「エラー」等）
  - [ ] 受信音量メーター（Peak/RMS）をメイン画面に昇格
  - [ ] 出力後レベルをメイン画面に昇格
  - [ ] パケット統計（Packets/Loss/Jitter/Delay）をメイン画面に昇格
  - [ ] IP/ポート/CABLE状態は控えめに（折りたたみ or 小さく表示）

- [ ] 送信モードのメイン画面
  - [ ] 接続先IP入力 + 接続状態を大きく表示
  - [ ] 開始/停止ボタンをわかりやすく配置
  - [ ] 送信入力レベル（Peak/RMS）をメイン画面に昇格
  - [ ] 送信ステータスを視覚的に表示

- [ ] 設定ウィンドウの整理
  - [ ] 「情報」タブからメーター/統計を削除（メインに移動したため）
  - [ ] 「情報」タブはトラブルシューティングガイドに特化
  - [ ] 受信/送信の詳細設定はそのまま設定ウィンドウに残す

- [ ] `dotnet build` で確認
- [ ] ユーザー動作確認

### Phase2: 見た目のモダン化
目的: WinFormsの「Windows95感」を脱却。フラット・ダーク寄りのモダンデザインに。

- [ ] テーマシステムの導入
  - [ ] `UiTheme` 静的クラスを作成（カラー/フォント/余白の一元管理）
  - [ ] ダーク系カラーパレット定義
    - 背景: ダークグレー系 (#1E1E1E / #2D2D30 / #3E3E42)
    - 前景: ライトグレー (#CCCCCC / #FFFFFF)
    - アクセント: ブルー系 (#007ACC)
    - 状態色: 正常=緑(#4EC9B0)、警告=黄(#DCDCAA)、エラー=赤(#F44747)、無接続=グレー(#808080)
  - [ ] フォント定義（Segoe UI / メイリオ、サイズ3段階: 見出し/本文/補助）
...
- [ ] コントロールのフラットUI化
  - [ ] ボタン: `FlatStyle.Flat` + カスタムカラー + ホバー色
  - [ ] モード切替ボタン: 選択中/非選択の色分けを明確に
  - [ ] ComboBox/TextBox: 背景色・ボーダー色をテーマに合わせる
  - [ ] CheckBox: テーマカラー適用
  - [ ] TrackBar: 可能な範囲でカスタム（または描画カスタム）
  - [ ] GroupBox/Label: テーマカラー適用

- [ ] 余白・レイアウトの最適化
  - [ ] Padding/Margin の統一ルール策定（8px基準のグリッド）
  - [ ] セクション間の余白を統一
  - [ ] ラベル幅の最適化
