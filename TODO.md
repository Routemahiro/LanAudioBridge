...
### Phase3: タスクトレイ常駐 + 自動接続 (案B実装: 2026-02-07)
目的: 「セットアップしたら放置」を実現。起動→自動接続→トレイ格納の一連の流れ。

- [x] NotifyIcon でタスクトレイ常駐 (2026-02-07 完了)
  - [x] トレイアイコンの追加（アプリアイコン）
  - [x] 右クリックコンテキストメニュー（表示/設定/終了）
  - [x] 最小化時にトレイに格納（タスクバーから消える）
  - [x] トレイアイコンダブルクリックで復帰
  - [x] トレイアイコンのツールチップに接続状態を表示

- [x] 自動接続機能 (2026-02-07 完了)
  - [x] `AppSettings` に `AutoConnect` (bool) プロパティ追加
  - [x] 設定ウィンドウに「動作」タブ追加 + 「起動時に自動接続する」チェックボックス
  - [x] 起動時に `AutoConnect=true` なら自動で前回モードで接続開始
...
- [x] エラー状態のUI表示改善 (2026-02-07 完了)
  - [x] エラーメッセージのパターン整理（接続切断/デバイス消失/ポート競合等）
  - [x] 「何が起きたか」+「どうすればいいか」をセットで表示
  - [x] ShowAlert の表示改善（ステータスバーに対処法も表示）

- [x] トレイ通知（バルーン） (2026-02-07 完了)
  - [x] 接続切断時にバルーン通知
  - [x] エラー発生時にバルーン通知
  - [x] 再接続成功時にバルーン通知
  - [x] 同一メッセージの連続通知抑制ロジック（10秒間隔）

- [x] `dotnet build` で確認 (2026-02-07 完了)
- [x] ユーザー動作確認 (2026-02-07 完了)

### バグ修正: 再起動ボタン押下時のエラー（案B: Mutexフラグ管理 + Process.Start）
- [x] Program.cs: Mutex を static フィールドに変更し、外部から解放可能にする (2026-02-07 完了)
