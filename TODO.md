...
- [x] テーマシステムの導入 (2026-02-06 完了)
  - [x] `UiTheme` 静的クラスを作成（カラー/フォント/余白の一元管理）
  - [x] ダーク系カラーパレット定義
    - 背景: ダークグレー系 (#1E1E1E / #2D2D30 / #3E3E42)
    - 前景: ライトグレー (#CCCCCC / #FFFFFF)
    - アクセント: ブルー系 (#007ACC)
    - 状態色: 正常=緑(#4EC9B0)、警告=黄(#DCDCAA)、エラー=赤(#F44747)、無接続=グレー(#808080)
  - [x] フォント定義（Segoe UI、サイズ4段階: 見出し/本文/補助/インジケーター）

- [x] フォームのダークモード化 (2026-02-06 完了)
  - [x] Form背景色/前景色をテーマカラーに
  - [x] 全コントロールに再帰的にテーマ適用

- [x] コントロールのフラットUI化 (2026-02-06 完了)
  - [x] ボタン: `FlatStyle.Flat` + カスタムカラー + ホバー色
  - [x] モード切替ボタン: 選択中=アクセントブルー/非選択=ダークグレー
  - [x] ComboBox/TextBox: 背景色・ボーダー色をテーマに合わせる
  - [x] CheckBox: テーマカラー適用
  - [x] TrackBar: 背景色のみテーマ適用（カスタム描画は見送り）
  - [x] GroupBox/Label: テーマカラー適用
  - [x] TabControl: オーナー描画でダークタブヘッダー

- [x] 余白・レイアウトの最適化 (2026-02-06 完了)
  - [x] Padding/Margin を8px基準グリッドに統一
  - [x] セクション間の余白を統一（SpaceXs=4/SpaceSm=8/SpaceMd=16/SpaceLg=24）
  - [x] 行高さを8px倍数に調整（26→32等）

- [x] StatusStrip のスタイル改善 (2026-02-06 完了)
  - [x] 背景色をテーマに合わせる（DarkColorTable）
  - [x] アラート時の色をStateErrorに統一

- [x] SettingsFormのダーク化 (2026-02-06 完了)
